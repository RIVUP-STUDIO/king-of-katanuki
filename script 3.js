(function(){
  ...
  function furinRadius(theta, Rb){ // 風鈴: round glass bell + one hanging strip
    ...
    r += Rb * 0.42 * bump(theta, 90, 11); // tanzaku strip hanging straight down
    return r;
  }
  function uchiwaRadius(theta, Rb){ // うちわ: round paddle + straight handle
    let r = Rb * (1 + 0.10*Math.cos(2*(theta + Math.PI/2)));
    r += Rb * 0.42 * bump(theta, 90, 9); // handle
    return r;
  }
  function goldfishRadius(theta, Rb){ // 金魚: oval body + forked tail
    let r = Rb * (1 + 0.15*Math.cos(2*theta));
    r += Rb * 0.46 * bump(theta, 180, 35);
    r -= Rb * 0.24 * bump(theta, 180, 9);
    r += Rb * 0.22 * bump(theta, 160, 8);
    r += Rb * 0.22 * bump(theta, 200, 8);
    return r;
  }
  ...
  function buildStageCache(){
    ...
    // inside it — floor keeps it visually dominant, clamp keeps it on-canvas.
    plateR = Math.min(W*0.47, Math.max(maxR + safeBand + W*0.16, W*0.44));
  }
  ...
  function spawnChipFragment(bucket, now){
    ...
    const originR = innerR + (plateR - innerR) * (0.35 + Math.random()*0.3);
  ...
  function draw(){
    ...
    // instead of the whole plate fading uniformly.
    ...
    if(shapePts.length === N_BUCKETS){
      for(let i = 0; i < N_BUCKETS; i++){
        ...
        if(chipStartTime[i] > 0){
          ...
          outerR = plateR - (plateR - innerR) * cpEase;
        }
